...
import com.opensource.dbhelp.dbutils.ResultSetHandler;

/**
 * <p>
 * <code>ResultSetHandler</code> implementation that returns a Map. <code>ResultSet</code> rows are converted into objects (Vs) which are then stored in a Map under the given keys (Ks).
 * </p>
 *
 * @param <K>
...
public abstract class AbstractKeyedHandler<K, V> implements ResultSetHandler<Map<K, V>> {
	...
	/**
	 * Convert each row's columns into a Map and store then in a <code>Map</code> under <code>ResultSet.getObject(key)</code> key.
	 * 
	 * @param rs
	 ...
	 * @see com.opensource.dbhelp.dbutils.ResultSetHandler#handle(java.sql.ResultSet)
	 */
	@Override
	...
	/**
	 * This factory method is called by <code>handle()</code> to create the Map to store records in. This implementation returns a <code>HashMap</code> instance.
	 *
	...
	/**
	 * This factory method is called by <code>handle()</code> to store the current <code>ResultSet</code> row in some object.
	 * 
	 * @param rs
